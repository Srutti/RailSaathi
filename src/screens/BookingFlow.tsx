...
import { createHold, onHoldExpired, confirmHold } from '../services/hold-service';
import { assignPNR, updateSession } from '../services/booking-session';
...
const BookingFlow: React.FC = () => {
  ...
  useEffect(() => {
    const cb = (holdId: string) => {
      if (currentHold?.holdId === holdId) {
        setHoldExpired(true);
        if (stage !== 'CONFIRMED' && stage !== 'PAYMENT') {
          setError({
            code: 'HOLD_EXPIRED',
            message: 'Your seat hold has expired. If you already paid, we\'re checking the status — don\'t pay again.',
            recoverable: stage === 'PAYMENT',
            holdPreserved: stage === 'PAYMENT',
          });
        }
      }
    };
    onHoldExpired(cb);
  }, [currentHold, stage, setHoldExpired, setError]);
  ...
  }, [currentHold, session, bid, setPaymentRecord, setStage, setPNR, setError]);
  ...
  const handleNewBooking = useCallback(() => {
    resetBooking();
    if (session) {
      updateSession(session.sessionToken, { stage: 'SEARCHING' });
    }
